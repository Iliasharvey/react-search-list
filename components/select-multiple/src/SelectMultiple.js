...
const itemType = PropTypes.oneOfType([
  PropTypes.string,
  PropTypes.shape({ label: PropTypes.any, value: PropTypes.any, subkey: PropTypes.any, image: PropTypes.any })
])
...
export default class SelectMultiple extends Component {
  ...
  onRowPress (row) {
    const { label, value, subkey, image } = row
    ...
    if (index > -1) {
      ...
      selectedItems = selectedItems.concat({ label, value, subkey, image})
    }

    this.props.onSelectionsChange(selectedItems, { label, value, subkey, image })
  }

  toLabelValueObject (obj) {
    if (Object.prototype.toString.call(obj) === '[object String]') {
      return { label: obj, value: obj, subkey: obj, image: obj}
    } else {
      return { label: obj.label, value: obj.value, subkey: obj.subkey, image: obj.image }
    }
  }
  ...
  renderLabel = (label, subkey, image, style, selected) => {
    if (this.props.renderLabel) {
      return this.props.renderLabel(label, subkey, image, style, selected)
    }
    return (
    ...
    return (
      <TouchableWithoutFeedback onPress={() => this.onRowPress(row)}>
        <View style={rowStyle}>
          ...
          {this.renderLabel(row.label, row.subkey, row.image, labelStyle, row.selected)}
        </View>
